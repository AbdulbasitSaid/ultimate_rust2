...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!( target:"hop", "a Frog hopped, it has {:#?} of energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target:"hop","frog will go to sleep since he ran out of energy");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target:"sleep","(non-fatal) error stating that the Frog is already asleep");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target:"Frog","default value was generated, with the debug representation: {:#?}", frog);
        frog
    }
}
